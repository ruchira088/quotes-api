...
class GoodReadsQuotationExplorer[F[_]: Concurrent](client: Client[F]) extends QuotationExplorer[F] {
  ...
  private val visitedUris: ConcurrentHashMap[Uri, Unit] = new ConcurrentHashMap[Uri, Unit]()

  override val discover: Stream[F, DiscoveredQuote] = {
    Stream
      .eval(logger.infoF("Starting discovery..."))
      .productR(Stream.eval(run(uri"/quotes")))
      .flatMap { document =>
        Stream
          .fromEither[F]
          .apply(GoodReadsQuotationExplorer.tags(document))
          .flatMap {
            _.foldLeft[Stream[F, DiscoveredQuote]](Stream.empty) {
              case (stream, (name, uri)) =>
                Stream
                  .eval(logger.infoF(s"Discovering quotes about $name"))
  ...
  def readAllQuotes(name: String, pageUri: Uri): Stream[F, DiscoveredQuote] = {
    Stream
      .eval(Sync[F].delay(Option(visitedUris.get(pageUri)).nonEmpty))
      .flatMap { hasVisited =>
        if (hasVisited) {
          Stream.eval(logger.infoF(s"Has already visited: $pageUri"))
            .productR {
              readAllQuotes(
                name,
                pageUri.copy(
                  query = pageUri.query.removeQueryParam("page") ++ List(
                    "page" -> Some(pageUri.query.params.get("page").flatMap(_.toIntOption).getOrElse(1) + 1).map(_.toString)
                  )
                )
              )
            }
          ...
          Stream
            ...
            .flatMap { document =>
              ...
              Stream
                .eval(Sync[F].delay(visitedUris.put(pageUri, (): Unit)).as(quotes))
                .flatMap(values => Stream.emits(values)) ++
                nextPage.fold[Stream[F, DiscoveredQuote]](
                  error => Stream.raiseError[F](error),
                  _.fold[Stream[F, DiscoveredQuote]](
                    Stream.eval(logger.infoF(s"Finished discovery for $name")).productR(Stream.empty)
                  ) { uri =>
                    readAllQuotes(name, uri)
                  }
                )
            }
      }
  ...
  def run(uri: Uri): F[Document] =
    logger.infoF(s"Making request: GET $uri").productR {
      client
        .expect[String](GoodReadsQuotationExplorer.Host.resolve(uri))
        .flatMap { html =>
...
object GoodReadsQuotationExplorer {
  ...
  def nextPage(document: Document): ParseResult[Option[Uri]] =
    Option(document.select("a.next_page").first())
      .map(_.attr("href"))
      .filter(_.nonEmpty)
      .fold[ParseResult[Option[Uri]]](Right(None)) { href =>
        Uri.fromString(href).map(Some.apply)
      }

  def tags(document: Document): ParseResult[List[(String, Uri)]] =
    document
      .select(".bigBox li a.gr-hyperlink")
      .asScala
      .toList
      .map { element =>
